...
export default function ManageCategoriesPage() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: "1.5rem", marginBottom: "2rem" }}>Manage Categories</h2>
      ...
      <div style={{ background: "#fff", padding: "1.5rem", borderRadius: 10, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", maxWidth: 480, marginBottom: "2.5rem" }}>
        <h3 style={{ marginTop: 0, fontSize: "1.1rem", marginBottom: "1.25rem" }}>{editId ? "Edit Category" : "New Category"}</h3>
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: "1rem" }}>
            <label style={{ display: "block", fontSize: "0.9rem", fontWeight: 500, marginBottom: "0.4rem" }}>Name</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Politics" required
      ...
      {loading ? <p style={{ color: "#6b7280" }}>Loading…</p> : (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", background: "#fff", borderRadius: 10, overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
            ...
            <tbody>
              {categories.length === 0 ? (
                <tr><td colSpan={4} style={{ padding: "2rem", textAlign: "center", color: "#9ca3af" }}>No categories yet</td></tr>
              ) : categories.map(cat => (
                <tr key={cat.id} style={{ borderTop: "1px solid #f3f4f6" }}>
                  <td style={{ padding: "0.75rem 1rem", fontWeight: 500 }}>{cat.name}</td>
                  <td style={{ padding: "0.75rem 1rem", color: "#6b7280", fontFamily: "monospace", fontSize: "0.85rem" }}>{cat.slug}</td>
                  <td style={{ padding: "0.75rem 1rem", color: "#6b7280", fontSize: "0.85rem" }}>{cat.description || "—"}</td>
